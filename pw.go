...
func passwordReset(c *gin.Context) {
	...
	content := T(c,
		"Hey %s! Someone, which we really hope was you, requested a password reset for your account. In case it was you, please <a href='%s'>click here</a> to reset your password on The Realm. Otherwise, silently ignore this email.",
		username,
		config.BaseURL+"/pwreset/continue?k="+key,
	)
	msg := mailgun.NewMessage(
		config.MailgunFrom,
		T(c, "The Realm password recovery instructions"),
		content,
		email,
	)
	msg.SetHtml(content)
	...
	addMessage(c, successMessage{T(c, "Done! You should shortly receive an email from us at the email you used to sign up on The Realm.")})
	getSession(c).Save()
	c.Redirect(302, "/")
}
